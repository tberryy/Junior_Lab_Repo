...
function plotYearlyFracturePoints( ...
    ...
    for y = 1:length(yearsToPlot)
        ...
        for m = 1:length(materials)
            ...
            %% ========================================================
            %% STEP 4 — polynomial best fit of the average
            %%
            %% The fit uses the pooled pre-failure points of every
            %% retained specimen, so it is a true least-squares average
            %% rather than a smoothed pointwise mean.
            %% ========================================================
            ...
            if numel(pooledX) > polynomialOrder + 1

                fitOrder = min(polynomialOrder, numel(pooledX) - 1);

                coefficients = polyfit(pooledX, pooledY, fitOrder);

                %% Report fit quality
                predictedPooled = polyval(coefficients, pooledX);
                ...
                %% Evaluate from the start of the data to the mean
                %% failure strain, so the curve ends on the failure line
                fitStart = min(pooledX);
                fitEnd   = meanFailureX;

                if fitEnd > fitStart

                    fitX = linspace(fitStart, fitEnd, numberOfGridPoints)';
                    fitY = polyval(coefficients, fitX);

                    %% The average failure point lies ON the fitted curve
                    curveFailureY = polyval(coefficients, meanFailureX);

                    fprintf(['  %s: order %d fit, R^2 = %.4f | ' ...
                             'curve at mean failure strain = %.4g ' ...
                             '(mean of failure stresses = %.4g)\n'], ...
                        materialName, fitOrder, rSquared, ...
                    ...
                    %% ------------------------------------------------
                    %% Stress spread about the fit at each x location.
                    %% Each specimen is interpolated onto the fit grid
                    %% and the pointwise SD is taken.
                    %% ------------------------------------------------
                ...
                warning('Not enough pooled points to fit %s %s %s.', ...
                    yearNumber, materialName, testName);
            end
            ...
            if ~isempty(fitX)
                ...
                legendLabels(end+1)  = materialName + ...
                    string(sprintf(' order-%d fit (n = %d)', ...
                        polynomialOrder, numberKept));                   %#ok<AGROW>
        ...
        subtitleText = string(sprintf( ...
            'Order-%d least-squares fit | outliers: %s', ...
            polynomialOrder, outlierMethod));
